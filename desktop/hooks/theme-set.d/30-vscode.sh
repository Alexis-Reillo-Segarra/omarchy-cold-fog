#!/bin/bash
# OMARCHY_THEME_HOOK_GUARD: omarchy-hook (3.8+) tambien recorre theme-set.d/ por su
# cuenta, sin las variables de color que exporta ~/.config/omarchy/hooks/theme-set.
# Sin esto los hooklettes corren 2 veces y la 2a pasada escribe colores vacios.
...
# El tema de VS Code se deriva SIEMPRE de colors.toml, aunque el tema de Omarchy
# traiga su propio vscode.json apuntando a una extension de terceros (p.ej.
# Solitude -> Noctokai). Asi los colores del editor siguen a los del sistema.

# vs-dark o vs-light segun la luminancia relativa (WCAG) del fondo del tema
theme_type="dark"
if awk -v h="$primary_background" 'BEGIN{
        for (i = 0; i < 3; i++) {
            c = strtonum("0x" substr(h, i * 2 + 1, 2)) / 255
            v = (c <= 0.03928) ? c / 12.92 : exp(log((c + 0.055) / 1.055) * 2.4)
            l += v * (i == 0 ? 0.2126 : (i == 1 ? 0.7152 : 0.0722))
        }
        exit !(l > 0.18)
    }'; then
    theme_type="light"
...
# La plantilla base16 asume que color0 es un gris algo mas claro que el fondo.
# En temas como Solitude color0 == background, asi que la seleccion y el
# resaltado de linea quedarian invisibles. Derivamos un tono apenas separado.
shade_clamped() {
    local hex=$1 delta=$2 c out=""
    for i in 0 2 4; do
        c=$((16#${hex:$i:2} + delta))
        ((c < 0)) && c=0
        ((c > 255)) && c=255
...
if [[ $theme_type == "light" ]]; then
    subtle_bg=$(shade_clamped "$primary_background" -10)
else
    subtle_bg=$(shade_clamped "$primary_background" 12)
fi

# Texto legible sobre un fondo de acento. La plantilla base16 asume acentos
# saturados y oscuros, pero paletas grises (Solitude) los tienen claros: fijar
# bright_white encima daba 1.76:1. Elegimos fondo o primer plano segun cual
# contraste mas con el acento concreto.
best_fg() {
    awk -v bg="$1" -v a="$2" -v b="$3" '
        function lum(h,   i, c, v, l) {
            for (i = 0; i < 3; i++) {
                c = strtonum("0x" substr(h, i * 2 + 1, 2)) / 255
                v = (c <= 0.03928) ? c / 12.92 : exp(log((c + 0.055) / 1.055) * 2.4)
                l += v * (i == 0 ? 0.2126 : (i == 1 ? 0.7152 : 0.0722))
        ...
        function cr(x, y,   l1, l2, t) {
            l1 = lum(x) + 0.05; l2 = lum(y) + 0.05
            if (l2 > l1) { t = l1; l1 = l2; l2 = t }
            return l1 / l2
        }
        BEGIN { print (cr(bg, a) >= cr(bg, b)) ? a : b }'
}

on_accent=$(best_fg "$normal_blue" "$primary_background" "$primary_foreground")
on_green=$(best_fg "$normal_green" "$primary_background" "$primary_foreground")
on_yellow=$(best_fg "$normal_yellow" "$primary_background" "$primary_foreground")
on_red=$(best_fg "$normal_red" "$primary_background" "$primary_foreground")
on_bright_red=$(best_fg "$bright_red" "$primary_background" "$primary_foreground")
on_magenta=$(best_fg "$normal_magenta" "$primary_background" "$primary_foreground")

# La pestana activa usa el acento del tema (active_tab_background en colors.toml
# cuando existe) para poder distinguirla; con la plantilla original todas las
# pestanas quedaban del mismo color que el fondo.
active_tab=$(awk '$1 == "active_tab_background" && /=/ {
        if (match($0, /#([0-9a-fA-F]{6})/)) { print substr($0, RSTART + 1, 6); exit }
    }' "$HOME/.local/state/omarchy/current/theme/colors.toml")
[[ -z $active_tab ]] && active_tab="$subtle_bg"
on_active_tab=$(best_fg "$active_tab" "$primary_background" "$primary_foreground")

# Siempre se regenera: colors.toml es la unica fuente de verdad
if true; then
    cat > "$output_file" << EOF
{
    ...
    "button.secondaryForeground":"#${on_magenta}",
    ...
    "tab.unfocusedActiveBackground":"#${subtle_bg}",
    "tab.activeForeground":"#${on_active_tab}",
    ...
    "editor.selectionBackground":"#${selection_background}59",
    "editor.inactiveSelectionBackground":"#${selection_background}33",
    "editor.selectionHighlightBackground":"#${selection_background}33",
    "editor.wordHighlightBackground":"#${selection_background}33",
    ...
    "editor.lineHighlightBackground":"#${subtle_bg}",
    ...
    "statusBarItem.remoteForeground":"#${on_green}",
    "statusBarItem.errorBackground":"#${normal_red}", //8
    "statusBarItem.errorForeground":"#${on_red}",
    "statusBarItem.warningBackground":"#${normal_yellow}", //A
    "statusBarItem.warningForeground":"#${on_yellow}",
...
# Extension local y propia: no depende de la red ni de que una extension de
# terceros conserve un manifest parcheado tras actualizarse.
extension_id="omarchy.omarchy-theme"
ui_theme="vs-dark"
[[ $theme_type == "light" ]] && ui_theme="vs"

install_theme_into() {
    local ext_root="$1"
    [[ -d $ext_root ]] || return 0

    local ext_dir="$ext_root/${extension_id}-1.0.0"
    ...
    cat > "$ext_dir/package.json" << EOF
{
  "name": "omarchy-theme",
  "displayName": "Omarchy",
  "description": "Generado desde ~/.local/state/omarchy/current/theme/colors.toml. No editar a mano.",
  "publisher": "omarchy",
  "version": "1.0.0",
...
EOF
    ...
    cp "$output_file" "$ext_dir/themes/omarchy-color-theme.json"

    # Registrar en el cache de extensiones para que VS Code lo vea al arrancar
    local cache="$ext_root/extensions.json"
    if [[ -f $cache ]]; then
        local updated
        updated=$(jq --arg id "$extension_id" --arg rel "${extension_id}-1.0.0" --arg abs "$ext_dir" '
...
# Fija el tema del editor al generado desde el sistema, preservando el resto
# de ajustes del usuario (settings.json es JSONC, por eso jq va en modo lento).
pin_theme_setting() {
    local settings="$1"
    [[ -f $settings ]] || return 0
    ...
    updated=$(jq '
        ...
        | ."workbench.preferredDarkColorTheme" = "Omarchy"
        | ."workbench.preferredLightColorTheme" = "Omarchy"
    ' "$settings") || {
        warning "No se pudo actualizar $settings (JSON invalido); se deja intacto."
        return 0
    }
    printf '%s\n' "$updated" > "$settings"
}

install_theme_into "$HOME/.vscode/extensions"
install_theme_into "$HOME/.vscode-oss/extensions"

pin_theme_setting "$HOME/.config/Code/User/settings.json"
pin_theme_setting "$HOME/.config/Code - OSS/User/settings.json"
pin_theme_setting "$HOME/.config/VSCodium/User/settings.json"
